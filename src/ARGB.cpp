 ...
 * @return Scaled value
 */
static inline u8_t scale8(u8_t x, u8_t scale) { return ((uint16_t)x * scale) >> 8; }
...
static void ARGB_TIM_DMADelayPulseCplt(DMA_HandleTypeDef *hdma)
{
    ...
    else if (BUF_COUNTER < NUM_PIXELS + 2)
    { // if RET transfer
...
// Check channel
#if !(TIM_CH == TIM_CHANNEL_1 || TIM_CH == TIM_CHANNEL_2 || TIM_CH == TIM_CHANNEL_3 ||             \
